...
def zenodo_statistics_hp(filename= "", data = None, stats = ['pie_on_conditions', 'hist_on_functions']):
    ...
    if('pie_on_conditions' in stats):
        ...
        lconditions = [val for val in lconditions if val is not None]
        if len(lconditions) > 0:
            ...
            # Create a placeholder figure/file
            plt.figure()
...
def zenodo_map(filename = "",data = None, levels = [1, 2, 3], country_base_url = "https://raw.githubusercontent.com/eamena-project/eamena-data/main/reference_data/countries/", grid_contour_url = "https://raw.githubusercontent.com/eamena-project/eamena-arches-dev/main/dbs/database.eamena/data/reference_data/grids/EAMENA_Grid_contour.geojson", label='Heritage Places', edgecolor='black', linewidth=1, fig_size_width=10, fig_size_height=10):
  ...
  for level in levels:
    ...
    if level == 2:
        ...
        # Extract country names safely
        for feature in data['features']:
            country_raw = feature['properties'].get('Country Type', None)
            if country_raw and isinstance(country_raw, str):
                # Strip trailing commas and whitespace
                country_clean = country_raw.strip().strip(',')
                # Only add if non-empty
                if country_clean == "Palestine, State of":
                    country_clean = "Palestine"
                if country_clean == "Iran (Islamic Republic of)":
                    country_clean = "Iran"
                if country_clean:
                    lcountries.add(country_clean)

        # Now attempt to plot each country, but handle errors gracefully
        for country in lcountries:
            try:
                country_url = f"{country_base_url}{country}.geojson"
                gdf_country = gpd.read_file(country_url)
                ...
                # Log or handle the error; for now just print a warning and continue
                print(f"Warning: Could not load country data for '{country}'. Error: {e}")
                continue
