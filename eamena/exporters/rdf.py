    ...
    def get_rdf_graph(self):
        ...
        def add_edge_to_graph(graph, domainnode, rangenode, edge, tile, graph_info):
            ...
            if type(pkg["d_uri"]) == list:
                ...
                # This is a hack because the to_rdf function doesn't check for missing ids or properties and will crash if not present.
                # We just insert garbage because the to_rdf function deletes them anyway.
                if 'range_tile_data' in pkg:
                    if isinstance(pkg['range_tile_data'], dict):
                        if 'features' in pkg['range_tile_data']:
                            if isinstance(pkg['range_tile_data']['features'], list):
                                for f in pkg['range_tile_data']['features']:
                                    if not('id' in f):
                                        f['id'] = ''
